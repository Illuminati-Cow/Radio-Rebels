extends Node
# Modified version of rsubtil's demo player_manager.gd
# player is 0-3
...
signal player_input_event_start(player_id)
...
# Get the device associated with the specified player.
# WARNING: Returns NAN if the player does not exist or has no device associated.
func get_player_device(player: int) -> int:
	# Prevent a crash from implicitly casting null to NAN. Explicit cast instead.
	var result = get_player_data(player, "device")
	return result if result != null else NAN
